...
export default function ControllerEventDetails() {
  ...
  const { servedImageUrl, isImageLoading } = useEventImage({ eventId: 1, token: token?.toString() });

  const { selectedEvent, isEventLoading } = useAdminEvent({ eventId: 1, token: token?.toString() });
  ...
  return (
    <div className="bg-primary-black min-h-screen flex flex-col text-primary-white pt-10 rounded-lg p-4 items-center">
      ...
      <ControllerPanelSection token={token} eventId={1} />
    </div>
  )
}