...
const TicketButtons = ({ tickets, updateQuantity }: { tickets: Ticket[], updateQuantity: (id: string, increment: boolean) => void }) => {
  return (
    <div className="space-y-2 mb-6">
        {tickets.map((ticket) => (
          <div key={ticket.id} className="flex flex-wrap gap-x-5 gap-y-4 bg-cards-container px-3.5 py-3 rounded-lg items-center justify-center xs:justify-between">
            <div className='w-[100px] sm:w-[170px]'>
              <div className="font-semibold text-center xs:text-start text-body">{ticket.type}</div>
            ...
            <div className="flex items-center font-light text-subtitle">
              <button
                onClick={() => updateQuantity(ticket.type, false)}
              ...
              <button
                onClick={() => updateQuantity(ticket.type, true)}
