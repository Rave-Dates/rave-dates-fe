...
export default function CreateCategoryValues() {
  ...
  const [selectedPreviewCategory, setSelectedPreviewCategory] =
    useState<number>(1);
  ...
  const handleUpdate = (valueId: number, newValue: string) => {
    updateMutate({
      ...
      valueId,
      value: newValue,
      categoryId: selectedPreviewCategory,
    });
  };
  ...
  const currentPreviewCategory = categories?.find(
    (c) => c.categoryId === selectedPreviewCategory,
  );
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col items-start my-3">
        <p className="w-34 mb-3 text-sm font-medium">
          Valores de categorías activos:
        </p>

        <div className="relative w-full">
          <label
            htmlFor="preview-category-select"
          ...
          <div className="relative">
            <select
              ...
              onChange={(e) =>
                setSelectedPreviewCategory(Number(e.target.value))
              }
        ...
        <div className="flex items-center flex-wrap gap-x-4 gap-y-2 mt-4 ms-2">
          ...
          currentPreviewCategory.values.length > 0 ? (
            ...
            <p className="text-sm text-text-inactive italic">
              No hay valores para esta categoría
            </p>
          )}
        </div>
      </div>
