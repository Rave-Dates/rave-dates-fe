...
const EventDetails = ({ eventId }: { eventId: number }) => {
  ...
  useEffect(() => {
    if (urlPromotorAffiliate) {
      setCookie("promoterAffiliate", urlPromotorAffiliate, {
        path: "/",
        maxAge: 60 * 60 * 12,
      });
    }
  ...
  return (
    <div className="min-h-screen bg-primary-black text-white pb-20 lg:pt-[6.7rem]">
      <div className="max-w-7xl mx-auto pt-0 pb-20 md:px-6 md:py-8 animate-fade-in">
        ...
        <div className="hidden md:grid grid-cols-2 gap-x-8">
          ...
          <div className="space-y-8">
            ...
            <TicketSelector
              eventInfo={{ piggyBank: selectedEvent?.piggyBank }}
              maxPurchase={selectedEvent?.maxPurchase}
          ...
          <div className="px-6">
            ...
            <div className="mb-8">
              ...
              <p className="text-body capitalize bg-cards-container px-4 py-3 rounded-lg">
                {selectedEvent?.date && (
                  <>
                    {formatDateToColombiaTime(selectedEvent.date).formatted}{" "}
                    {formatDateToColombiaTime(selectedEvent.date).time}hs (COL)
                  </>
                )}
              </p>
            </div>
