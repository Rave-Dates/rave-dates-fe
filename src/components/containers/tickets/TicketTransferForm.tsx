...
const TicketTransferForm = ({ purchaseTicketId } : { purchaseTicketId: number }) => {
  ...
  return (
      <DefaultForm ticketButtons={true} handleSubmit={handleSubmit(onSubmit, onInvalid)} title="Tickets propios">
        <div className="flex flex-col w-full gap-y-2">
          {
            isEventLoading ?
              ...
              :
              selectedEvent && 
                <TitleCard title={selectedEvent.title} description={selectedEvent.subtitle}>
                  {
                    isImageLoading ?
        ...
        <FormInput
          ...
          inputName="idCard"
          register={register("idCard", { required: "La cédula o pasaporte es obligatoria" })}
        />
        <FormInput
          type="email"
          title="Email*"
